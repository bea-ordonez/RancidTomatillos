...
class SingleMovie extends Component {
  ...
  componentDidMount = () => {
    fetchPromises(`movies/${this.props.movieId}`)
      .then((data) =>
        this.setState({ singleMovieSelected: data.movie, loading: false })
      )
  ...
  getMovieTrailer = () => {
    if (this.state.singleMovieVideos.length) {
      const trailerMovie = this.state.singleMovieVideos.find(
    ...
    if (this.state.loading) {
      ...
      return (
        <div
          ...
          style={{
            backgroundImage: `url("${this.state.singleMovieSelected.backdrop_path}")`,
          }}
        >
          <NavLink to="/">
          ...
          <div className="movie-details">
            ...
            <p>
              Rating: {this.state.singleMovieSelected.average_rating}/ 10 🍅{" "}
            </p>
